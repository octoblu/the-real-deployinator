...
class DeployinatorPostpone
  parseOptions: =>
    ...
    [@projectName, @tag, @postponeSeconds] = commander.args

    unless @projectName? && @tag? && @postponeSeconds?
      return @die "projectName, tag, deployAt is required: received '#{@projectName}', '#{@tag}', '#{@postponeSeconds}'"
  ...
  deploy: =>
    ...
    deployAt = Math.floor((Date.now() / 1000)) + (@postponeSeconds * 1000)
    ...
    request.post options, (error, response, body) =>
      ...
      debug 'response', body
      console.log 'Postponed', colors.yellow @tag
      console.log ""
